...
-- Requer a migration 2026_09_15_000001_make_importacao_imagem_nullable.
SET NAMES utf8mb4;
START TRANSACTION;

SET @idioma_id = (SELECT id FROM idiomas WHERE padrao = 1 ORDER BY id DESC LIMIT 1);

INSERT INTO importacoes (visivel, ordem, criado)
SELECT 1, 0, NOW() FROM DUAL WHERE @idioma_id IS NOT NULL;

INSERT INTO importacoes_idiomas (pais, descricao, idioma_id, importacao_id, criado)
SELECT 'França',
...
INSERT INTO importacoes (visivel, ordem, criado)
SELECT 1, 1, NOW() FROM DUAL WHERE @idioma_id IS NOT NULL;
...
INSERT INTO importacoes (visivel, ordem, criado)
SELECT 1, 2, NOW() FROM DUAL WHERE @idioma_id IS NOT NULL;
